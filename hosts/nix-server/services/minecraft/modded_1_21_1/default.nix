...
let
  mods = pkgs.stdenv.mkDerivation {
    name = "mods";
    src = pkgs.fetchurl {
      url = "https://cloud.ole.blue/public.php/dav/files/RaxCELdG4bTJoj2/?accept=zip";
  ...
  connect = pkgs.writeShellScript "connect.sh" ''
    ${lib.getExe' pkgs.tmux "tmux"} -S ${config.services.minecraft-servers.runDir}/modded_1_21_1.sock attach
  '';
