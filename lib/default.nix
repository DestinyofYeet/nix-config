...
rec {
  ...
  vm = import ./vm { inherit inputs pkgs lib; };
  ...
  settings = {
    ...
    screenshot-cmd = ''${pkgs.grim}/bin/grim -g "$(${pkgs.slurp}/bin/slurp -w 0)" - | ${pkgs.satty}/bin/satty -f - --early-exit --fullscreen -o /tmp/screenshot.png --copy-command ${pkgs.wl-clipboard}/bin/wl-copy'';
