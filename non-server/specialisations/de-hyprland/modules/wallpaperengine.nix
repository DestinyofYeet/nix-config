...
lib.mkIf (osConfig.capabilities.wallpaperEngine.enable) {
  services.linux-wallpaperengine = let
    mkIgnoreSteamApps = apps:
      builtins.concatStringsSep " "
      (map (app: "--fullscreen-pause-ignore-appid ${app}") apps);
    ...
    wallpapers = [
      {
        ...
        extraOptions = [ "--fullscreen-pause-only-active" ignoredSteamApps ];
        monitor = "DP-3";
      }
      {
